cordova.define("cordova-plugin-qrscanner.QRScannerProxy", function(require, exports, module) { /* eslint-disable no-redeclare, no-unused-vars */
...
module.exports = function () {
  ...
  function chooseCameras() {
    ...
    return devices.then(function (mediaDeviceInfoList) {
      ...
      var backCamera = null,
          frontCamera = null; // choose backCamera

      for (var i = 0; i < bestToWorstCameras.length; i++) {
        if (bestToWorstCameras[i].facingMode === "environment") {
          backCamera = bestToWorstCameras[i]; // (shouldn't be used for frontCamera)
      ...
      if (backCamera === null) {
        if (bestToWorstCameras.length > 0) {
          backCamera = bestToWorstCameras[0]; // (shouldn't be used for frontCamera)
  ...
  function startCamera(success, error) {
    ...
    window.navigator.mediaDevices.getUserMedia({
      ...
      video: {
        ...
        width: {
          ideal: currentCamera.width
        },
        height: {
          ideal: currentCamera.height
        }
      }
    }).then(function (mediaStream) {
      activeMediaStream = mediaStream;
      var video = getVideoPreview(); // Older browsers may not have srcObject
  ...
  function initialize(success, error) {
    if (scanWorker === null) {
      ...
      var original__webpack_public_path__ = __webpack_require__.p;
      var worker_public_path = window.QRSCANNER_WORKER_PUBLIC_PATH || "/plugins/cordova-plugin-qrscanner/src/";

      if (worker_public_path !== null) {
        __webpack_require__.p = worker_public_path;
      }

      scanWorker = new Worker(new URL(
      /* webpackChunkName: 'worker' */
      /* worker import */ __webpack_require__.p + __webpack_require__.u(348), __webpack_require__.b));
    ...
    if (!getVideoPreview()) {
      ...
      videoPreview.setAttribute("style", "display:block;position:fixed;top:50%;left:50%;" + "width:auto;height:auto;min-width:100%;min-height:100%;z-index:" + ZINDEXES.preview + ";-moz-transform: translateX(-50%) translateY(-50%);-webkit-transform: " + "translateX(-50%) translateY(-50%);transform:translateX(-50%) translateY(-50%);" + "background-size:cover;background-position:50% 50%;background-color:#FFF;");
  ...
  function scan(success, error) {
    ...
    show(function (ignore) {
      ...
      var video = getVideoPreview();
      var returned = false;
      scanning = true; // processing boolean prevents sampling new images when we're in the middle of processing.
      ...
      scanWorker.onmessage = function (event) {
        ...
        if (obj.result) {
          ...
          var SCAN_INTERVAL = window.QRScanner_SCAN_INTERVAL || 130; // this value can be adjusted on-the-fly (while a scan is active) to
          // balance scan speed vs. CPU/power usage

          nextScan = window.setTimeout(thisScanCycle, SCAN_INTERVAL);

          _cancelNextScan = function cancelNextScan(sendError) {
            window.clearTimeout(nextScan);
            nextScan = null;
            _cancelNextScan = null;
      ...
      thisScanCycle = function thisScanCycle() {
        if (processing) {
          return;
        }
  ...
  function pausePreview(success, error) {
    ...
    if (mediaStreamIsActive()) {
      ...
      bringStillToFront(); // kill the active stream to turn off the privacy light (the screenshot
...
/******/ 			if (chunkId === 348) return "../src/browser/worker.min.js";
...
!function() {
var cordovaRequire = __webpack_require__(833);

var cordovaModule = __webpack_require__(694);

var createQRScannerInternal = __webpack_require__(428);
